...
const SRD_FULL_DOC = 'daggerheart-srd/.build/01_pdf/DH-SRD-2025-09-09.md';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^# [A-Z]/)) {
      ...
      if (currentSection && currentSection.content.length > 0) {
        ...
        if (cleanContent.length > 50) { // Skip very small chunks
          chunks.push({
    ...
    if (currentSection) {
      ...
      // Content at top level without ## (like intro text)
      if (!currentSection) {
        currentSection = {
...
async function indexDaggerheart() {
  ...
  // Index individual files (classes, ancestries, etc.)
  let indexed = 0;
  ...
  try {
    ...
    for (const chunk of chunks) {
      try {
        ...
        store.upsert(chunk.id, chunk.content, chunk.category, embedding);
        chunkIndexed++;
